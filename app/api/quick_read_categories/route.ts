...
export async function POST(request: NextRequest) {
  try {
    ...
    const category = await Category.create({ name: name.trim(), createdBy: userId })
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, message: 'Category and its resources deleted' }, { status: 200 })
  } catch (error) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!id || !newName) {
       return NextResponse.json({ success: false, message: 'ID and new name are required' }, { status: 400 })
    }
    ...
    await QuickRead.updateMany({ category: oldName }, { $set: { category: newName.trim() } })
