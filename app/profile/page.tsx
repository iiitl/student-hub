  ...
  useEffect(() => {
    if (session?.user?.email) {
      const checkPasswordStatus = async () => {
        try {
          const response = await fetch(`/api/user/password-status?email=${encodeURIComponent(session.user?.email as string)}`)
          if (response.ok) {
      ...
      <div className="bg-gradient-to-br from-primary/5 to-primary/10 rounded-xl p-8 mb-8">
        ...
        {passwordSet === false && (
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/30 text-amber-700 dark:text-amber-400 px-6 py-4 rounded-md mb-6 flex items-start">
            ...
            <div>
              <p className="font-medium">Your account doesn't have a password set</p>
              <p className="text-sm mt-1">
                You're currently signed in with Google, but you can also set a password to access your account with email and password.
              </p>
              <Link 
        ...
        <div className="bg-background rounded-lg shadow-md p-6">
          ...
          <div className="space-y-4">
            {passwordSet ? (
              <Link href="/auth/change-password" className="flex justify-between items-center p-4 bg-muted/30 rounded-md hover:bg-muted/50 transition-colors">
              ...
              <Link href={`/auth/set-password?email=${encodeURIComponent(session.user?.email || '')}`} className="flex justify-between items-center p-4 bg-muted/30 rounded-md hover:bg-muted/50 transition-colors">
                <div className="flex items-center">
                  <div className="mr-3 h-10 w-10 rounded-full bg-gradient-to-br from-primary/20 to-primary/30 flex items-center justify-center">
                    <Settings className="h-5 w-5 text-primary" />
                  </div>
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">Set a password for email login</p>
                  </div>
                </div>
              </Link>
            )}
            ...
            >
              <div className="flex items-center">
                ...
                <div>
                  <p className="font-medium text-red-600 dark:text-red-400 text-left">Sign Out</p>
