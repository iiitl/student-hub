        ...
        {message.replyTo && (
          ...
          >
            <div className="font-semibold text-[10px] mb-1">
              Replying to {typeof message.replyTo.sender === 'object' && message.replyTo.sender !== null ? (message.replyTo.sender as Sender).name : 'someone'}
            </div>
            <div className="truncate opacity-75 max-w-[150px]">
              {typeof message.replyTo.content === 'string' && message.replyTo.content === '🚫 This message was deleted.'
                ? 'Deleted message'
                : typeof message.replyTo.content === 'string' ? message.replyTo.content : ''}
            </div>
          </div>
        )}

        {/* Message Bubble + Actions */}
        <div className={`flex items-center gap-1 relative z-10 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
          <div
